...
- `outlook_list_events_delta` — List only event changes since last call within a window (massive token savings for recurring agent jobs); each changed event carries `type` and `show_as`, the same keys `outlook_list_events` returns
