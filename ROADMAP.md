...
- **Inbox message rules (`/me/mailFolders/inbox/messageRules`)** — Graph's docs list personal-account support for these endpoints (`MailboxSettings.ReadWrite` for [Create rule](https://learn.microsoft.com/en-us/graph/api/mailfolder-post-messagerules?view=graph-rest-1.0) and [Delete messageRule](https://learn.microsoft.com/en-us/graph/api/messagerule-delete?view=graph-rest-1.0); `MailboxSettings.Read` for [List rules](https://learn.microsoft.com/en-us/graph/api/mailfolder-list-messagerules?view=graph-rest-1.0)), but that's not borne out in practice: every operation returns `403 ErrorAccessDenied` on a real outlook.com/hotmail.com mailbox, with `MailboxSettings.ReadWrite` granted and consented. Verified 2026-08-25 with a raw HTTP probe against the live API (not just this project's client), so it isn't an SDK or auth-flow bug. Probe token scopes: `Calendars.ReadWrite`, `Mail.Read`, `Mail.ReadWrite`, `MailboxSettings.ReadWrite`, `User.Read`.

  As a side investigation, classic desktop Outlook's COM/MAPI object model (`Application.Session` → `Store.GetRules()`) reaches the same underlying rule store outside of Graph, and isn't subject to that block — round-tripped a rule created via COM against the real outlook.live.com web Rules UI to confirm. But it has its own, narrower wall: creating a *new* rule with a `MoveToFolder` action consistently fails (tested via default and custom target folders, with and without conditions, via dynamic and early-bound COM dispatch, and both immediately and after a delay, in case of propagation lag) — while `Delete` and `Stop` actions work and sync to the server fine. `Delete` turns out to be a special case of `MoveToFolder` internally (targeting Deleted Items), which is presumably why it's allowed where a manually-set arbitrary folder isn't. Reads and edits to *existing* move-to-folder rules (created outside COM, e.g. via the web UI) work fine — only creating/repointing one via automation is blocked. Reasonable guess: a deliberate restriction on personal accounts specifically for silently redirecting someone's incoming mail (a real phishing/account-takeover vector), separate from whichever restriction blocks Graph — but unconfirmed. Re-investigate if Microsoft changes either the Graph or MAPI behavior for personal accounts.

  Rules CRUD itself is a dead end, but `/me/inferenceClassification/overrides` — the one rule-shaped thing that *does* work on personal accounts — isn't: this repo already ships three tools over it (`outlook_set_inbox_override` and friends) for per-sender Focused/Other routing. No folder moves, but it's worth knowing about before concluding nothing in this space is reachable.

---

## Done

- **1.14.0** — Migration to the `mcp` 2.x SDK (`FastMCP` → `MCPServer`), lifting the `<2` ceiling 1.13.1 pinned as a stopgap. Client-invisible: all 62 tool schemas serialize byte-for-byte identically, and the annotations + `OUTLOOK_MCP_TOOLSETS` gating from 1.12.0 carry over unchanged. Drops the `_mcp_server` private-API workaround in favour of `MCPServer`'s native `version` kwarg. Python floor unchanged (`>=3.10`). No tool-count change.
- **1.13.0** — Correctness release. Three shipped bugs fixed, all invisible to a fully green mock suite: `$orderby` + any non-date `$filter` returned `400 InefficientFilter`, breaking `outlook_list_inbox`'s `from_address` and `classification` on every call (#31); `outlook_list_thread` hit the same rule and had **never worked in a released version**; and `sanitize_kql` stripped `:`, so every documented KQL property restriction returned 200 with zero results (#30). Added the live query-shape test tier (`pytest -m live`) that catches this class — mutation-verified — plus repaired the integration tier, which had been silently skipping since it was written (`AuthManager.login()` doesn't exist; the `AttributeError` was swallowed into a skip). Adds `uncategorized_only` on `outlook_list_inbox` (thanks @jasond727) and documents inbox message rules as non-viable on personal accounts (thanks @tlewthwa). No tool-count change.
