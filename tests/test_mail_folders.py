...
class TestCreateFolder:
    async def test_create_top_level(self):
        ...
        result = await create_folder(mock_client, name="My Folder")
    ...
    async def test_create_nested(self):
        ...
        result = await create_folder(
            mock_client, name="Sub Folder", parent_folder="AAMkAGparent123="
        )
    ...
    async def test_create_raises_read_only(self):
        ...
        with pytest.raises(ReadOnlyError):
            await create_folder(mock_client, name="Test", read_only=True)
...
class TestRenameFolder:
    async def test_rename_validates_id_and_patches(self):
        ...
        result = await rename_folder(
            mock_client, folder_id="AAMkAGfolder123=", name="Renamed Folder"
        )
    ...
    async def test_rename_rejects_invalid_id(self):
        ...
        with pytest.raises(ValueError):
            await rename_folder(mock_client, folder_id="", name="New Name")

    async def test_rename_raises_read_only(self):
        """rename_folder raises ReadOnlyError in read-only mode."""
        mock_client = MagicMock()
        with pytest.raises(ReadOnlyError):
            await rename_folder(mock_client, folder_id="AAMkAG123=", name="New", read_only=True)
...
class TestDeleteFolder:
    async def test_delete_validates_id_and_deletes(self):
        ...
        result = await delete_folder(mock_client, folder_id="AAMkAGfolder456=")
