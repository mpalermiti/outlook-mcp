...
#
# 11,811 after the calendar `timezone` arguments (+183: two parameters and the
# lines explaining what a zone anchor does to a recurring series). That leaves
# 189 tokens, not the ~700 this number was set with — the next docstring fix
# will redden this guard. Deliberately left alone rather than swept upward:
# re-baselining a budget is the owner's call, and a ceiling raised as a
# side effect of the change that consumed it stops being a budget.
